...
function resolveConfiguredRealtimeVoiceProvider(params) {
	...
	if (!resolution.ok && resolution.code === "missing-configured-provider") throw new Error(`Realtime voice provider "${resolution.configuredProviderId}" is not registered. The realtime voice stack (Discord/voice-call) needs a registered provider id in voice.realtime.provider. For push-to-talk chat, install the Miracle Claw voice module from Settings → Modules.`);
	if (!resolution.ok && resolution.code === "no-registered-provider") throw new Error(params.noRegisteredProviderMessage ?? "No realtime voice provider registered. Configure voice.realtime.provider in OpenClaw, or use the Miracle Claw voice module (Settings → Modules) for push-to-talk chat.");
	if (!resolution.ok) throw new Error(`Realtime voice provider "${resolution.provider?.id}" is not configured. Configure its provider block in voice.realtime.providers, or switch voice.realtime.provider to a different registered provider.`);
	return {
