...
- New Tauri command `mc_voice_native_capture` (Windows-only via `#[cfg(windows)]` + `#[cfg(not(windows))]` stub pair) that spawns `mc-voice-native-capture.ps1`. The PS1 helper uses `System.Speech.Recognition` (SAPI 5) to listen on the default Windows mic, recognizes the user's speech, and returns JSON `{ text, confidence, engine }`.
- New client-side patch `mc-voice-native.js` injected into `control-ui/index.html` before `</body>`. Intercepts clicks on `.chat-send-btn--voice` / `.chat-send-btn--stop` in capture phase (before React's synthetic handler), blocks the original cloud Realtime Talk `getUserMedia` path, and routes to `mc_voice_native_capture` instead. Result text is written into the composer input via React-aware value setter + dispatched `input` / `change` events so React notices the change.
- 10-50× faster than whisper.cpp sidecar on a desktop CPU: ~200-500 ms native SAPI 5 vs ~3-5 s whisper.cpp decode + sidecar spawn. Free for all users, no module install required.
- Privacy line held: audio never leaves the device. SAPI 5 is fully offline. PS1 runs in a constrained `System.Speech.Recognition` context with no write access.
- New `installer-assets/mc-voice-native-capture.ps1` bundled as a `tauri.conf.json` resource so it ships to `$INSTDIR\resources\` at install time.

#### Add-On Modules catalog expansion
- `miracle-claw-github` (GitHub Helper) — flipped from `coming_soon` to `available` in `src/pages/modules.js`. Hook location: Developer.
- `miracle-claw-sql` (SQL Buddy) — flipped from `coming_soon` to `available`. Hook location: Developer.
- Both modules are installable from the Add-On Modules area in the same flow as Voice/OCR/Calendar.

#### patch-openclaw-dist.sh improvement (Lesson 707)
- `*.html.insert` patches now support multiple `<!-- MC-PATCH: <id> -->` markers in a single file. The script extracts ALL markers, checks which are missing from the target, and injects only the missing-marker sections. Marker lines are preserved in the target so idempotency works on subsequent runs.

#### Permissions / ACL fix-up
- `voice-diagnostics`, `voice-open-sound-settings`, `voice-open-windows-update`, and `mc-voice-native-capture` are now properly registered in `capabilities/main.json`, `permissions/default.toml`, and `permissions/autogenerated/commands/app_commands.toml`. Previous rc54.x builds had the first three missing from the autogenerated file due to a sccache-cached Tauri 2 ACL regeneration — closed that gap so JS calls are no longer silently rejected.
...
- **Fix: Files page ACL (`mc_ui_list_allowed_roots not allowed by ACL`).**
  v1.0.9-rc46 shipped with the three new Tauri commands registered in
  `generate_handler!` but missing from the Tauri 2 ACL manifests. All
  five UI commands are now properly listed in `capabilities/main.json`,
  `permissions/default.toml`, and `permissions/autogenerated/commands/app_commands.toml`.
- **Fix: Notebook UX — "+ New" silently discarded unsaved typing; no
  clear after save.** +New now auto-commits typed text into the active
  note before creating the new one. New **Save & New** button persists
  the active note and immediately creates a fresh blank one.
- **New: Files preview upgrades (v1.0.0-prep).**
  - **Image inline preview**: PNG, JPG, GIF, WebP, SVG, BMP, ICO render
    inline as data URIs via new `mc_ui_read_image` Rust command (5 MB cap).
  - **"Open in default app" handoff**: every preview pane now has a button
    that hands the file to the OS shell (Windows `cmd /c start`, macOS
    `open`, Linux `xdg-open`) via new `mc_ui_open_externally` Rust
    command. Path allowlist enforced the same way as `mc_ui_read_file`.
  - **Binary-file detection**: known binary extensions (PDF, Office,
    ZIP, MP4, fonts, etc.) skip the read entirely and show a "MC can't
    preview this — open in your default app" panel. Files with unknown
    extensions that look binary (high U+FFFD replacement ratio after
    UTF-8 decode) get the same fallback.
  - **Line numbers in text previews**: replaces the bare `<pre>` with a
    gutter+line display, similar to GitHub / VSCode. Big readability win
    for newbies scrolling through code or config files.
  - New `mc_ui_read_image` + `mc_ui_open_externally` commands require
    Tauri ACL entries; both are registered alongside the existing
    `mc_ui_*` commands.

### Things built this session (WIP — pending rc47 rebuild)

- **Fix: Files tile ACL (`mc_ui_list_allowed_roots not allowed by ACL`).**
  v1.0.9-rc46 shipped with the three new Tauri commands (`mc_ui_list_allowed_roots`,
  `mc_ui_list_dir`, `mc_ui_read_file`) wired into `generate_handler!` but NOT
  registered in `capabilities/main.json`, `permissions/default.toml`, or
  `permissions/autogenerated/commands/app_commands.toml`. Tauri 2 silently
  rejects any unlisted command at runtime (Lesson 219 trap). Files page is
  blocked until ACL is updated.
  - All three command identifiers `allow-mc-ui-list-allowed-roots`,
    `allow-mc-ui-list-dir`, `allow-mc-ui-read-file` are now listed in both
    the default permission set and the main window capability.
  - The `autogenerated/.../app_commands.toml` was hand-patched because the
    rc46 build's incremental sccache cache skipped Tauri's autogeneration
    step (the file still listed only rc36 entries). The next clean build
    will over-write this file; the patch is just so rc47 ships unlocked.
  - Files: `src-tauri/capabilities/main.json`, `src-tauri/permissions/default.toml`,
    `src-tauri/permissions/autogenerated/commands/app_commands.toml`.

- **Fix: Notebook UX — "+ New" silently discarded unsaved typing; no clear
  after save.** Two related issues found by David on rc46:
  - Clicking **+ New note** while the editor had un-saved typing now
    *auto-commits* the typed text into the currently-active note before
    creating a new blank one. (`notebook.js`: dirty check + manual
    persist in the +New handler.)
  - New **Save & New** button: persists the active note and immediately
    creates a fresh blank note, ready for the next entry. Save alone
    still leaves the active note selected for further editing.
  - Both buttons sit in the bottom-right of the editor; the chain reads
    Save, then Save & New, with Delete as a separate low-emphasis action
    on the left.
  - Files: `src/pages/notebook.js`.

### Things built this session (WIP — pending rc46 rebuild)

- **Dashboard expanded to five tiles: OpenClaw (Windows / Terminal), Files, Notebook, Local Terminal.**
  v1.0.9-rc45 split OpenClaw into Windows + Terminal. rc46 adds two more
  lanes: a Files browser and a Notebook. The dashboard is now a real
  AI workspace, not a launcher.
  - **Files** (`#files-tile`, `pages/files.js`): browse Documents, Desktop,
    Downloads, and the MC workspace. Click a directory to descend; click a
    file to preview it (UTF-8, capped at 512 KB by default; truncation is
    surfaced as a badge). New Rust commands `mc_ui_list_allowed_roots`,
    `mc_ui_list_dir`, `mc_ui_read_file` reuse the path-allowlist enforcement
    that the model-side tools already use, so the user-facing browser and
    the AI share the same safety boundaries. Backed by the live
    `crate::tools::exec::allowed_roots()` / `normalize_user_path` /
    `assert_path_allowed` helpers.
  - **Notebook** (`#notebook-tile`, `pages/notebook.js`): local Markdown
    notes stored in `localStorage` under `mc.notebook.v1`. Up to 200 notes;
    auto-save on blur; manual Save button; New / Delete actions. Per-install
    only in v1; future hook to MAIC's user_notes table if the user signs in.
  - **main.js**: registers `files` and `notebook` pages; `pageCtx()` gains
    `onOpenFiles` and `onOpenNotebook`. Tile IDs renamed for clarity:
    `terminal-tile` → `local-terminal-tile` (matches the visible label).
  - **CSS**: added styles for `.files-page` and `.notebook-page`, plus a
    shared `button.primary` helper used by Notebook's Save / New buttons.
  - Files: `src/main.js`, `src/pages/dashboard.js`, `src/pages/files.js`
    (new), `src/pages/notebook.js` (new), `src/styles.css`,
    `src-tauri/src/lib.rs`, `package.json` (1.0.9-rc45 → 1.0.9-rc46).

### Things built this session (WIP — pending rc45 with Lesson 528 banner)

- **Dashboard tile split: OpenClaw · Windows / OpenClaw · Terminal / Local Terminal.**
  Previously the dashboard had two tiles: a single OpenClaw tile (webview) and a
  Terminal tile that secretly launched `mc-openclaw` because that was hardcoded as
  the OS default. Now: three explicit lanes.
  - **OpenClaw · Windows** (`#openclaw-windows-tile`) — existing webview path
    (`openclaw_open_window`); desktop chat in the Tauri webview.
  - **OpenClaw · Terminal** (`#openclaw-terminal-tile`) — new tile; mounts the
    terminal page with `defaultShell: "mc-openclaw"` so the user lands in the
    OpenClaw TUI.
  - **Local Terminal** (`#terminal-tile`, was "Terminal") — local shell launcher,
    now defaults to `cmd` (Windows) / `bash` (macOS/Linux). The shell picker is
    still available inside the page.
  - Terminal page accepts `ctx.defaultShell` override; resolution order is now
    ctx → localStorage → OS default. localStorage value `"mc-openclaw"` is
    deliberately treated as stale (falls through to the OS default) so pre-rc45
    installs don't keep landing in the TUI when they click Local Terminal.
  - main.js learned `onOpenOpenClawTerminal` in `pageCtx()`.
  - Files: `src/main.js`, `src/pages/dashboard.js`, `src/pages/terminal.js`,
    `package.json` (version bump to `1.0.9-rc45`).
