...
; "${MAINBINARYNAME}.exe"` (defined in utils.nsh) before the file copy.
; If MC is running, Tauri prompts to kill it (or kills silently in
; passive mode).
;
; However, `CheckIfAppIsRunning` only kills the `miracle-claw.exe`
; process tree. Orphaned `node.exe` processes left behind by a previous
; crash or forced kill keep their file handles — so the installer still
; hits "Error opening file for writing: node.exe" on upgrade.
;
; This hook runs BEFORE `CheckIfAppIsRunning`, killing any orphaned
; `node.exe` so the file copy lands cleanly. We also kill
; `miracle-claw.exe` belt-and-suspenders (Tauri will re-check next).
;
; `nsExec::ExecToLog` swallows error codes — if no such process is
; running, the install proceeds normally.
; ============================================================================
